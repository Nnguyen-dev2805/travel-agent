...
class ConversationCreate:
    ...
    Per ADR 0011 the conversation is owned directly by `owner_user_id` with
    an optional `workspace_id` compatibility association. A `None`
    workspace means a standalone conversation; no hidden or default
    ...
    def __post_init__(self) -> None:
        ...
        if self.workspace_id is None:
            object.__setattr__(self, "workspace_id", None)
        else:
            object.__setattr__(
...
@dataclass(frozen=True)
class Conversation:
    """One persisted conversation record owned directly by a user.

    Per ADR 0011 `owner_user_id` is the authorization owner and
    `workspace_id` is an optional compatibility association. A `None`
    workspace means a standalone conversation; no hidden workspace exists.
    """
