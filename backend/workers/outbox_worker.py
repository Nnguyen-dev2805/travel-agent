...
def process_outbox_events():
    ...
    while _running:
        ...
        try:
            # 1. Fetch exactly 1 pending or retriable failed event securely with SKIP LOCKED
            ...
            ).order_by(MemoryOutbox.created_at.asc()).limit(1).with_for_update(skip_locked=True)
            ...
            if event.status == "FAILED" and event.retry_count > 0:
                ...
                if time_since_update < required_backoff:
                    db.rollback()
                    time.sleep(1) # Prevent hot-looping over the same locked row that isn't ready
                    continue

            logger.info(f"Processing outbox event ID {event.id} ({event.action} -> {event.memory_id})")
            ...
            try:
                ...
                db.commit()
                logger.info(f"Successfully processed outbox event ID {event.id}")
