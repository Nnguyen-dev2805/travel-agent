...
| Basic Semantic Memory Write Pipeline | Owns versioned assertions, authority ranking, risk-based policy decisions, transactional outbox emission, deterministic conflict resolution, bounded model extraction with retry/repair, and background shadow worker; persisted to PostgreSQL with strict row-level locking and idempotency | `backend/memory/write_pipeline/models.py`, `backend/memory/write_pipeline/policy.py`, `backend/memory/write_pipeline/resolver.py`, `backend/memory/write_pipeline/command_service.py`, `backend/memory/write_pipeline/worker.py`, `backend/memory/write_pipeline/uow.py` |
| Memory Write Pipeline Evaluation | Replays 22 required scenarios across 16 mandatory slices, validates bilingual datasets, computes precision/recall/accuracy metrics, enforces 12 non-compensating hard gates, and generates JSON/MD reports with non-regression comparison | `backend/memory/write_pipeline/evaluation/runner.py`, `backend/memory/write_pipeline/evaluation/cli.py`, `backend/memory/write_pipeline/evaluation/dataset.py` |
