...
Observability and operations is the next runtime package. `R8` has an approved
version 0.2 spec, an accepted ADR 0009, and an approved implementation plan.
No observability source exists yet, and R8 is ready to hand off to an
implementation worker in an isolated worktree.
...
| `R8` | Observability and Operations | In progress | `R2`, `D6`, `R7`, [ADR 0009](../adr/0009-privacy-safe-local-observability-boundary.md) | Privacy-safe structured events, request correlation, readiness diagnostics, ops evaluation, and runbook integration | Operators can diagnose degraded memory, retrieval, model, storage, and planner paths without leaking secrets or raw user content | [R8 spec v0.2](../specs/2026-09-05-observability-and-operations-design.md) approved; [ADR 0009](../adr/0009-privacy-safe-local-observability-boundary.md) accepted; [implementation plan v0.3](../plans/2026-09-05-observability-and-operations-implementation.md) approved; implementation in progress in worktree `r8-observability`: Tasks 1-6 done (contracts, events, middleware, instrumentation, readiness, evaluation); ops report `r8-operational-readiness-v0.1` at result `PASS` over 14 examples; owner review pending |
