...
| `R6` | Memory Retrieval | Delivered | `R5`, [ADR 0007](../adr/0007-feature-gated-memory-retrieval-and-context-boundary.md) | Feature-gated memory retrieval into bound chat context through orchestration-owned composition | Retrieval, scope, lifecycle, and safety evidence can be evaluated before any default-on personalization claim | [R6 spec v0.3](../specs/2026-09-04-memory-retrieval-design.md) approved and accepted through owner review; [ADR 0007](../adr/0007-feature-gated-memory-retrieval-and-context-boundary.md) accepted 2026-09-05; [implementation plan v0.1](../plans/2026-09-04-memory-retrieval-implementation.md) completed after owner acceptance; integration base `89496eb` verified at `834 passed`; Tasks 1-8 implemented in worktree `r6-retrieval`; review findings fixed in commit `5e84fd5`; final focused R6 verification reported `95 passed`, import-boundary checks clean, and retrieval report `r6-retrieval-v0.1` at result `PASS`; personalization and constraint-delta metrics remain `INCONCLUSIVE` without a provider-backed judge; Git delivery completed on `feature/agent-memory` at `d62b41b` |
| `R7` | Trip Planner State | Delivered | `R3`, `R4`, `R2`, `R6`, [ADR 0008](../adr/0008-workspace-owned-planner-state-and-operation-log.md) | Itinerary versions, trip decisions, and planner operations | Planner writes are explicit, reversible, and evaluated | [R7 spec v0.2](../specs/2026-09-05-trip-planner-state-design.md) approved 2026-09-05; [ADR 0008](../adr/0008-workspace-owned-planner-state-and-operation-log.md) accepted 2026-09-05; [implementation plan v0.2](../plans/2026-09-05-trip-planner-state-implementation.md) completed after owner acceptance; Tasks 1-8 implemented in worktree `r7-planner` on base `3a53dfa`; the review round moved every operation row into the same repository transaction as its state change, added `BEGIN IMMEDIATE` version allocation and two rollback regression tests, and introduced `ItineraryVersionDraft`, recorded in commit `5212117`; final verification reports `985 passed` with one known non-R7 failure (`test_chunker.py::test_loader_real_dataset` needs gitignored `data/processed/vietnam_travel_raw.jsonl`), `compileall` exit `0`, planner report `r7-state-v0.1` at result `PASS` over 16 examples with 6/6 gates, and clean import-boundary checks; delivered to the active local `feature/agent-memory` branch at `57e70fe` |
| `R8` | Observability and Operations | Accepted in working tree | `R2`, `D6`, `R7`, [ADR 0009](../adr/0009-privacy-safe-local-observability-boundary.md) | Privacy-safe structured events, request correlation, readiness diagnostics, ops evaluation, and runbook integration | Operators can diagnose degraded memory, retrieval, model, storage, and planner paths without leaking secrets or raw user content | [R8 spec v0.2](../specs/2026-09-05-observability-and-operations-design.md) approved 2026-09-05; [ADR 0009](../adr/0009-privacy-safe-local-observability-boundary.md) accepted 2026-09-05; [implementation plan v0.3](../plans/2026-09-05-observability-and-operations-implementation.md) completed after owner acceptance; Tasks 1-8 implemented in worktree `r8-observability` on base `e761f11`; the review round removed the unvalidated caller `conversation_id` from the chat accepted event so a malformed id keeps the `404` contract instead of raising through observability, extended redaction to `counters` entries while route paths survive, and corrected the architecture and security gap lists, recorded in commit `1413bee`; final verification reports `1036 passed` with one known non-R8 failure (`test_chunker.py::test_loader_real_dataset` needs gitignored `data/processed/vietnam_travel_raw.jsonl`), `compileall` exit `0`, ops report `r8-operational-readiness-v0.1` at result `PASS` over 14 examples with 6/6 gates, clean import-boundary and privacy-sentinel checks; Git delivery pending |
| `R9` | Security and Privacy Hardening | In progress | `D6`, `R3`, `R4`, `R6`, `R8`, [ADR 0010](../adr/0010-local-identity-authorization-and-deletion-boundary.md) | Local authenticated principal boundary, owner authorization, memory deletion semantics, controlled errors, request-size guard, and access boundaries | Privacy-sensitive flows have tests, security evaluation evidence, and refreshed memory hard-gate evidence | [R9 spec v0.2](../specs/2026-09-06-security-and-privacy-hardening-design.md) approved 2026-09-06; [ADR 0010](../adr/0010-local-identity-authorization-and-deletion-boundary.md) accepted 2026-09-06; [implementation plan v0.3](../plans/2026-09-06-security-and-privacy-hardening-implementation.md) approved 2026-09-06; all three gates are satisfied; implementation in progress in worktree `r9-security`: Tasks 1-9 done (auth, authorization, deletion, error hardening, both evaluations, docs); owner review found 2 P1 + 4 P2 blockers, fixed TDD in the same worktree (memory 409 to 404, tombstone-inclusive verifier, log-covering token gate, correlated size-config 500, stream restore, doc corrections); R9 report `r9-security-privacy-v0.1` at result `PASS` over 12 examples; refreshed `r6-retrieval-v0.2` closes both ordering gates at `0` events; scoped re-review pending |
